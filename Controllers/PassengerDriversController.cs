...
namespace nicenice.Server.Controllers
{
    ...
    public class PassengerDriversController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateLocation([FromBody] DriverLocationDto locationDto)
        {
            var driver = await _context.PassengerDrivers.FirstOrDefaultAsync(d => d.Id == locationDto.DriverId);
            if (driver == null)
                return NotFound("Driver not found.");

            var existing = await _context.DriverLocations.FirstOrDefaultAsync(dl => dl.DriverId == locationDto.DriverId);

            if (existing == null)
            {
                var newLoc = new DriverLocation
                {
                    Id = Guid.NewGuid(),
                ...
                _context.DriverLocations.Add(newLoc);
            }
            else
            {
            ...
            var ride = await _context.Rides
                ...
                .FirstOrDefaultAsync(r =>
                    r.DriverId == locationDto.DriverId &&
                    r.Status == RideStatus.Matched);

            if (ride != null && ride.Passenger != null)
            {
                var passengerGroupId = ride.Passenger.UserId.ToString();

                await _hubContext.Clients.Group(passengerGroupId).SendAsync("ReceiveRideUpdate", new
                {
                    rideId = ride.Id,
                    status = ride.Status,
                    driverLocation = new
